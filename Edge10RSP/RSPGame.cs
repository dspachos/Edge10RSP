    ...
    class RSPGame {
        ...
        static void PlayMatch(ref Match match) {
            ...
            while (match.NumberOfGamesPlayed < match.NumberOfTotalGames) {
                ...
                Write("Your move:");
                player1Move = GetUserInput(maxMoveRange) - 1;

                if (player1Move == -1) // User choose 0 to move to the main menu
                    break;
                ...
                switch (match.player2.PlayerType) {
                    case PlayType.manual:
                        ...
                        player2Move = GetUserInput(maxMoveRange) - 1;
                        break;
                    case PlayType.random:
                        player2Move = GetRandomMove(maxMoveRange);
                        break;
                    case PlayType.tactical:
                        player2Move = match.GetTacticalMove(maxMoveRange);
                        break;
                    default:
                        break;
                }
                WriteLine("You played: \t {0}", match.GetMoveLabel(player1Move));
                WriteLine("Player 2: \t {0} ", match.GetMoveLabel(player2Move));
                match.MovesHistory.Add(new Tuple < int, int > (player1Move, player2Move));
